...
st.markdown("**Professional Agent Evolution Engine** — Thorough academic-style reports")
...
# ====================== SOLVE WITH PROFESSIONAL REPORT ======================
def solve_with_strategy(problem, agent):
    ...
    try:
        ...
        report = f"# CogniEvo Technical Report\n\n"
        report += f"**Report ID:** {datetime.now().strftime('%Y%m%d_%H%M%S')}\n"
        report += f"**Generated by Agent:** {agent.name}\n\n"
        ...
        report += f"- **Confidence in Approach:** {random.randint(65, 95)}%\n\n"

        if agent.novel_strategies:
            latest = agent.novel_strategies[-1]
            report += f"## Invented Processing Method\n"
            report += f"**Method Name:** {latest['name']}\n"
            report += f"**Description:** {latest['description']}\n\n"

        report += f"## Step-by-Step Reasoning\n"
        report += f"1. Identified the core structure of the problem using {dominant.replace('_', ' ')} processing.\n"
        report += f"2. Applied pattern recognition and symmetry detection to reduce the search space.\n"
        if agent.novel_strategies:
            report += f"3. Utilized the newly invented method '{latest['name']}' to collapse redundant paths.\n"
        report += f"4. Verified the solution through consistency checks and edge case analysis.\n"
        report += f"5. Confirmed the result matches expected mathematical properties.\n\n"

        report += f"**Time Taken:** {duration:.3f} seconds\n"
        report += f"**Success:** {'Confirmed' if success else 'Partial exploration'}\n"
        report += f"**Report Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
        report += "---\n\n"
        report += f"*This report was autonomously generated by a cognitive agent in the CogniEvo system.*"
    ...
    for i, report in enumerate(reversed(st.session_state.user_profile["reports"])):
        with st.expander(f"Report #{len(st.session_state.user_profile['reports']) - i}", expanded=True):
            ...
            if st.button(f"📥 Download Report #{len(st.session_state.user_profile['reports']) - i} as .txt", key=f"download_{i}"):
                st.download_button(
                    label="Click to Download .txt (Open with Google Docs)",
                    data=report,
...
st.caption("CogniEvo v25 — Professional academic-style reports with step-by-step reasoning and proof")
