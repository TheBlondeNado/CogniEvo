...
st.set_page_config(page_title="CogniEvo v25 — Final", layout="wide")
...
def solve_with_strategy(problem, agent):
    ...
    try:
        ...
        report += f"**Dominant Mode:** {dominant.replace('_', ' ').title()}\n\n"
        ...
        report += f"**Time Taken:** {duration:.3f}s\n"
        report += f"**Success:** {'Yes' if success else 'Partial'}\n\n"
        report += f"**Answer:**\n{answer_str}"

        return success, duration, answer_str, report
    except:
        return False, 10.0, "Error", "Computation failed"
    ...
    if st.button("🚀 Evolve New Strategy Population", type="primary"):
        with st.spinner("Evolving agents..."):
            ...
            if champion.novel_strategies:
                st.subheader("🆕 Newly Invented Methods")
        ...
        with st.expander(f"#{i+1}: {prob['problem']}", expanded=False):
            ...
            if st.button("Test Current Evolved Strategy", key=f"test_{domain}_{i}"):
                if st.session_state.user_profile.get("personal_strategy"):
                    ...
                    st.warning("Evolve a strategy first using the sidebar.")

st.caption("CogniEvo v25 — Sidebar restored + detailed solved reports")
